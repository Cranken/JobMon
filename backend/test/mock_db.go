...
func (db *MockDB) GetJobData(j *job.JobMetadata, nodes []*job.Node, sampleInterval time.Duration, raw bool) (data job.JobData, err error) {
	db.Calls += 1
	return job.JobData{Metadata: j}, nil
}

func (db *MockDB) GetAggregatedJobData(j *job.JobMetadata, nodes []*job.Node, sampleInterval time.Duration, raw bool) (data job.JobData, err error) {
	db.Calls += 1
	return job.JobData{Metadata: j}, nil
}
