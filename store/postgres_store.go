...
func (s *PostgresStore) PutJob(job job.JobMetadata) {
	if job.TTL == 0 {
		job.TTL = s.config.DefaultTTL
	}
	s.db.NewInsert().Model(&job).Exec(context.Background())
}
...
func (s *PostgresStore) FetchJobMetadataData(job *job.JobMetadata) error {
